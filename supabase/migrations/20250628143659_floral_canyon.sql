/*
  ...
  1. New Tables
    - Ensures `leetcode_stats` table exists with proper structure
  2. Security
    - Sets up RLS policies for secure access
  3. Functions
    - Adds `sync_leetcode_to_user_stats` function
    - Creates `get_solved_problems_by_topic` function
  4. Triggers
    - Sets up trigger to sync LeetCode stats to user stats
*/
...
BEGIN
  IF EXISTS (
    ...
    RAISE NOTICE 'Table leetcode_stats is already in publication supabase_realtime';
  ELSE
    -- Table is not in the publication, add it
