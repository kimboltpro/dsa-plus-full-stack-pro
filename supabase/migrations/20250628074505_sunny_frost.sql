/*
  # Complete DSA Mastery Hub Database Schema

  1. New Tables
    - `profiles` - User profile information
    - `topics` - DSA topics for roadmap organization
    - `sheets` - Problem sheets (TUF, Striver, etc.)
    - `problems` - Individual coding problems
    - `user_progress` - User progress tracking
    - `bookmarks` - User bookmarked problems
    - `articles` - Educational articles and resources
    - `user_notes` - User notes on articles
    - `interview_questions` - Interview-specific questions
    - `user_stats` - User statistics and streaks

  2. Security
    - Enable RLS on all tables
    - Add policies for user data access
    - Add policies for public reference data access
  ...
  3. Features
    ...
    - Comprehensive DSA roadmap structure
*/
...
-- Insert sample sheets - using proper enum casting
DO $$
BEGIN
  -- Insert TUF Sheet
  IF NOT EXISTS (SELECT 1 FROM public.sheets WHERE name = 'TUF Sheet') THEN
    INSERT INTO public.sheets (name, description, sheet_type, total_problems, source_url) 
    VALUES ('TUF Sheet', 'Take U Forward comprehensive SDE interview preparation sheet', 'tuf'::sheet_type_enum, 191, 'https://takeuforward.org/interviews/strivers-sde-sheet-top-coding-interview-problems/');
  END IF;

  -- Insert Striver DSA Sheet
  IF NOT EXISTS (SELECT 1 FROM public.sheets WHERE name = 'Striver DSA Sheet') THEN
    INSERT INTO public.sheets (name, description, sheet_type, total_problems, source_url) 
    VALUES ('Striver DSA Sheet', '79 most important DSA problems for interview preparation', 'striver'::sheet_type_enum, 79, 'https://takeuforward.org/interviews/strivers-sde-sheet-top-coding-interview-problems/');
  END IF;

  -- Insert Love Babbar 450
  IF NOT EXISTS (SELECT 1 FROM public.sheets WHERE name = 'Love Babbar 450') THEN
    INSERT INTO public.sheets (name, description, sheet_type, total_problems, source_url) 
    VALUES ('Love Babbar 450', '450 most loved DSA problems curated by Love Babbar', 'love_babbar_450'::sheet_type_enum, 450, 'https://drive.google.com/file/d/1FMdN_OCfOI0iAeDlqswCiC2DZzD4nPsb/view');
  END IF;

  -- Insert Fraz Sheet
  IF NOT EXISTS (SELECT 1 FROM public.sheets WHERE name = 'Fraz Sheet') THEN
    INSERT INTO public.sheets (name, description, sheet_type, total_problems, source_url) 
    VALUES ('Fraz Sheet', 'Curated list of important DSA problems by Mohammad Fraz', 'fraz'::sheet_type_enum, 400, 'https://docs.google.com/spreadsheets/d/1-wKcV99KtO91dXdPkwmXGTdtyxAfk1mbPkQojjSZ0Ie/edit#gid=0');
  END IF;

  -- Insert GFG Top 100
  IF NOT EXISTS (SELECT 1 FROM public.sheets WHERE name = 'GFG Top 100') THEN
    INSERT INTO public.sheets (name, description, sheet_type, total_problems, source_url) 
    VALUES ('GFG Top 100', 'GeeksforGeeks top 100 coding problems for interviews', 'gfg_top_100'::sheet_type_enum, 100, 'https://www.geeksforgeeks.org/top-100-data-structure-and-algorithms-dsa-interview-questions-topic-wise/');
  END IF;

  -- Insert Company Specific
  IF NOT EXISTS (SELECT 1 FROM public.sheets WHERE name = 'Company Specific') THEN
    INSERT INTO public.sheets (name, description, sheet_type, total_problems, source_url) 
    VALUES ('Company Specific', 'Problems asked in top tech companies like Google, Amazon, Microsoft', 'company_specific'::sheet_type_enum, 500, null);
  END IF;
END $$;