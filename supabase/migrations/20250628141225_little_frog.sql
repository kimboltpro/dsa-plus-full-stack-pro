/*
  ...
  3. Indexes
    ...
    - Unique constraint on user_id and username combination

  4. Triggers
    - Add trigger to sync LeetCode stats to user_stats table when updated
*/

-- Create the leetcode_stats table
...
-- Add unique constraint for user_id and username combination
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE leetcode_stats ADD CONSTRAINT leetcode_stats_user_id_username_key UNIQUE (user_id, username);
  END IF;
END $$;
...
-- Create trigger to automatically sync stats
