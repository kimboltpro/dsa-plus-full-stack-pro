/*
  # Database Schema Migration for DSA Mastery Hub

  1. New Tables
     - `profiles` - User profile information
     - `topics` - DSA topic organization for roadmap
     - `sheets` - Problem sheet collections
     - `problems` - Individual coding problems
     - `user_progress` - User problem solving progress
     - `bookmarks` - User bookmarked problems
     - `articles` - Learning articles and resources
  ...
  3. Features
     ...
     - Sample data for topics and sheets
*/
...
-- Insert sample sheets - only if not exists, with proper enum casting
INSERT INTO public.sheets (name, description, sheet_type, total_problems, source_url) 
SELECT v.name, v.description, v.sheet_type::sheet_type_enum, v.total_problems, v.source_url
FROM (VALUES
